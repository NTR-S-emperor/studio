SAME FUNCTION AS INSTAPICS (except for the calling method)
Here, you can create OnlySlut posts using .txt files, for example "example.txt."
...
When you call up a post in Messenger (with $slut), be sure to specify the path from the "posts" folder. For example, if you have created an example.txt file in this path: "root\stories\[YOUR STORY]\onlyslut\posts\lovepath", the method for calling it will be "$slut = lovepath/example.txt".
